...
Language models encompasses a large family of neural network models with infinite architecture variations. Their goal is to perform a text related tasks like generating a summary, or in our case, classifying text inputs.
...
## Language Models of type Encoder{#sec-encoders}
...
The first BERT model was published by Google in 2019 (Devlin et al., 2019). "BERT" refers to a specific arrangement of Transformers layers with some tweaks that demonstrated great overall performance at the time and has become a reference in the realm of encoder models. As a consequence, many other BERT-like models were published, working off of these foundations, each new model proposing modifications or improvements like ModernBERT greatly increasing the context window size, CamemBERT trained on French texts or RoBERTa changing the training tasks. 
...
This pre-training is the most time and power consuming step, as the process can take from dozens of hours to several days, weeks, or even months (XXX source). This task is only performed by specialised organisations that have access to high-end hardware and large amounts of data (whether from legal or illegal sources). Once the process is done, the model can reach moderate to high performance on a large range of tasks.

## Fine-tuning

The "pre-training" implies there is a "training", or more specifically, a "fine-tuning". This fine-tuning process only takes a few minutes and can last a few dozens of minutes on more limited hardware. This process intends to adapt the model to **your** task and **your** data, going from moderate to, hopefully, very high performance. 
...
- Choose a model from HuggingFace and load it on your machine.
...
:::{.callout-note title="Model description" collapse="true"}
...
In @sec-training-parameters we introduced the main hyperparameters but did not provide much information on what they represent. In this section, we provide list these parameters and propose a description as well as a level of importance when tweaking the hyperparameters.

- **Learning rate** 🟢🟢🟢 _(importance)_
  - The learning rate represents the weights update rate at each step. More specifically, this hyperparameter represents the first learning rate value, as the learning rate itself changes during the process. Values too high might change the weights too much and mess with the model's inner balance; on the other hand values too low will not affect the model and the fine-tuning will have no effect.
  - Typical values: $[1e-6, 1e-4]$
- **Number of epochs** 🟢
  - This is the number of times the number model will see the full training set. You want it as low as possible[^number-of-epochs] but sufficient for the performance to peak.
  - Typical values: 3-5
- **Weight decay**[^alias-weight-decay] 🟢🟢
  - This prevents weights from growing too big (i.e. it's a regularisation parameter) and lead to overfitting. Values too low (meaning no regularisation) might result in overfitting whereas values too high may lead to underfitting.
  - Typical values: 0.01-0.1  
- **Batch size**, **Device batch size** and **Gradient accumulation steps** 🟢🟢
  - All three parameters monitor the number of inputs to use in a batch before updating the weights. The larger the batch the less sensitive to noise the updates are. However, depending on your hardware, you might not be able to reach high batch sizes. Therefore we set the Device batch size at a lower value and Accumulation steps so that $\text{Batch size} = \text{Device batch size} \times \text{Gradient accumulation steps}$.
  - Typical values for the Batch size: 16-64
- **Eval** and **Save strategies** 🔴
  - The strategies can be set to `"epoch"` or `"steps"`. If you use steps you can set the `eval_steps` and `save_steps` parameter to set after how many steps you want to evaluate your model and save it. Bear in mind that each model can weigh a few gigabytes, the more you save the more disk space you'll need. You can use `save_total_limit`[^note-save-total-limit] to limit the number of checkpoints saved.

[^number-of-epochs]: More epochs mean more computation time.
...
As mentionned in the introduction, one could choose to use an LLMs such as ChatGPT through an API ([illustrated here](https://www.css.cnrs.fr/classification-with-generative-llms-and-api-calls/)) or fine-tune an encoder model as illustrated in this tutorial. We list a number of pros and cons for using either technique in the context of social science research.
...
Alternatively, you can use [ActiveTigger](css.cnrs.fr/active-tigger), our open-source software designed to support collaborative text annotation and code-free model fine-tuning, and more! 

<!--TODO add logo-->

**Accessibility to a server**

Fine tuning an encoder model requires hardware and a clean environment as demonstrated throughout this tutorial; on the other hand, making API calls requires an API key with credit. This raises concerns regarding the equal access to such technologies.

**Control on the model**

Most models do not disclose the training data used for pretraining the model we use, this represent a hard limit when it comes to controlling the model used and the interpretability of the results. However, encoder models allow for more reproducibility (you own the model used for inference) and interpretability (you own the data used for fine tuning, XXX additional techniques to probe the model???).

**Open weights**

Most generative models cannot be downloaded on your personal device, these models are said "closed weights". If you want to use such models you will need to use an API to use this specific model. On the other hand, all models on Hugging Face (generative or encoder) can be downloaded and are said "open weights". This is critical for reproducibility purposes. (source with varying performance through after different model updates).

**Confidentiality**

If your research project requires extensive care regarding confidentiality, then you cannot use an LLM through API calls. You would need to run a model (generative or encoder) on trusted servers/devices. Therefore, fine-tuning an encoder lowers the bar for hardware requirements.
