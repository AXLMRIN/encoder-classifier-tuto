...
- Using decoder models: from a prompt, ie the concatenation of the text to annotate as well as the codebook, we ask a model to generate the labels as text. This strategy is easy to implement as few coding skills are required, and models run on external machines. This nonetheless has drawbacks, LIST THEM  <!-- TODO Reprendre les conclusions du tuto decoder -->
- Using encoder models: first annotate a sample of texts, from which the model is going to learn <REVOIR ICI, dire de manière moins complexe.>
...
Tokens are parts of words, whole words, or even punctuations and emojis. They are created using a [NAME OF THE ALGORITHM BYTE SOMETHING???](). They are used to subdivise the text and assign ready made embeddings that will be modified by the attention mechanism.
