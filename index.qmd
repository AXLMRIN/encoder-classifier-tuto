...
After presenting the objectives, materials and setup steps, we introduce the key NLP and Machine Learning concepts, we will break down the pipeline starting with loading the dataset and format it, then load the model and setting the finetuning parameters to finally launch the finetuning process. Once the finetuning is done, we will assess the performance and discuss how we can tweak the hyperparameters to reach optimum performance. We conclude this tutorial with advices on how to correctly incorporate the results in a social science project. Throughout the tutoriel we provide practical insights on how to best use your hardware.
...
- The conceptualisation of your research question to fall into the automatic annotation framework, we expect readers to be able to design their protocol and motivate it using appropriate social sciences and any domain speficic concepts.
...
In this tutorial we will **fine-tune** a **pretrained language model** of type **encoder** to perform a **classification** task. The description itself needs clarification, we will endeavor ourselves to shed light on all of these keywords while remaining on the surface level to keep this tutorial legible. At each step, we provide valuable resources to further understand the concepts.
...
Classification is part of the **supervised learning** family in machine learning: this means that to be able to assign labels, the algorithm will first need to be **fitted** onto some **training data**, i.e. handmade annotations. The fitting/training process consists of providing inputs and labels, so the algorithm can adapt to the task and _"learn"_. This mean that social scientists will first need to annotate a small set of texts in order to train the classifier. 

Classifiers often have structuring parameters called **Hyperparameters** that alter the algorithm and are not changed during training.
...
To correctly assess a model evaluation and prevent cherry-picking, we proceed to separate evaluation on distinct subsets of texts:
...
- The test set: these items will be used to assess the performance of the algorithm with the chosen hyperparameters on new data. Metrics on the test dev are is representative of the expected performance of the algorithm on new data. Scientists are encouraged to communicate on metrics computed on the test set to prevent from cherry picking the performance metrics. 

**Models**

In this context, models are [deep neural network models](https://en.wikipedia.org/wiki/Neural_network_(machine_learning))[^neural-networks]. Neural Network consist of connected nodes _(neurons)_ that receives signals from other neurons, processes the signal, and send the outcome to other neurons. Neurons are grouped into layers, and signals travel from the first layer to the last one transforming the input information into an output signal. A neural network is considered deep if it has more than 2 layers. Each neuron contains information called **weights** and **biases**. 
...
Ultimately, they can be seen as a long sequence of simple transformations, each transformation being conditioned by the **weights** and **biases**, that optimised to perform a certain task.
...
- **Decoder** models, including the GPT and Llama families, are generative models, meaning they are able to produce text. They are typically very large, with billions of weights that take up gigabytes of disk and memory space. While the smallest ones (a few billion weights) can be run on a laptop, today's frontier models use trillions of parameters and can only be run in datacenters, so that you can only use them through APIs. When these APIs are commercial, this can raise serious confidentiality issues for social science uses <!-- TODO reformulate -->. 
- **Encoder** models are intended to capture the semantic properties of their input text, which can then be used for downstream tasks. The output of an encoder model is a vector of several hundreds of dimensions called an **embedding**. They are much more frugal (below 1 billion parameters), so that in many cases they can be used on a laptop[^7]. 
...
The first BERT model was published by Google in 2019 (Devlin et al., 2019). "BERT" refers to a specific arrangement of Transformers layers with some tweaks that proved great overall performance at the time and has become a reference in the realm of encoder model. As a consequence, many other BERT-like models were published, working off of these foundations, each new model proposing modifications or improvements like ModernBERT being larger with a much greater context window size, CamemBERT trained on French data, RoBERTa changing the training process. 

**Pretrained Language Model**

We mentioned that language models were deep neural networks trained on a natural language task, but a model is not usable before it was trained to recognise language patterns and semantic. "Pretrained" refers to the training process that make a language model _understand_ language by training it to perform different tasks. In practice, the weights and biases are updated to optimize one or more tasks. For instance, BERT is trained on two tasks : 
