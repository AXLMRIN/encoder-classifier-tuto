...
- Evaluate models' performance and their impact on downstream tasks and discussing GPU issues <!-- JB: Le tuto ne mentionne pas vraiment "impact on downstream tasks" il me semble. => supprimer ?  AM: supprimer oui, on fera juste une mention dans la conclusion-->
...
**ES spotted many flaws in the structure. Will re-write this section entirely.**
...
<!-- TODO mix and match the paragraphs  -->

All models we discuss here are neural [network models](https://en.wikipedia.org/wiki/Neural_network_(machine_learning))[^neural-networks] with specific architectures (activation nodes, pooling layers or feedback loops). Ultimately, one can see models as a sequence of matrix multiplication and vectors addition where the coefficients of the matrices and vectors (the weights) have been optimised for a certain task. The Transformer architecture, is one way of agencing nodes together. In one of the most famous paper in NLP (["Attention is all you need"](https://en.wikipedia.org/wiki/Attention_Is_All_You_Need)), a team from Google introduced the concept of *attention* which attempts to take into account the context in which a word is used.  

All models presented here are deep neural networks. This is a very large family of algorithms, with infinite architecture variations and subtleties, but ultimately they can be seen as a long sequence of simple matrix operations (sums and multiplications) and nonlinear transformations, that map some input data into some output data. The specific sequence of operations in a model is called its architecture, and the matrix values that are used in the process of transforming the input data are called its weights. These weights are optimized during a training process, by minimizing a loss function that measures how well the model is performing for its given task. 

The Transformer architecture has become the de facto standard for contemporary artificial intelligence, in its many forms. It was introduced in 2017 by a team from Google, in perhaps the most influential paper in contemporary NLP and beyond, (["Attention is all you need"](https://en.wikipedia.org/wiki/Attention_Is_All_You_Need)). The cornerstone of this architecture is *self-attention*, which is an efficient way of taking into account the context in which a word is used.
