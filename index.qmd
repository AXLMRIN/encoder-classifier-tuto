...
- **Encoder** models are not generative, and are only intended to capture the semantic properties of their input text, which can then be used for downstream tasks. They include BERT and its hundreds of variations: RoBERTa, DeBERTa, ... their name usually gives them away. They are much more frugal (below 1 billion parameters), so that in many cases they can be used on a laptop[^7]. 
...
The first BERT model was published by Google in 
...
<!-- TODO distinction train et fine-tune -->
