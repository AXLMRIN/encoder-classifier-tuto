...
[^tokens-def]: Texts cannot be used directly and must undergo a tokenization process. Models come with a vocabulary of tokens, each text is therefore a combination of "tokens" from the vocabulary. [Read more](Jurafsky chapter on tokens)
...
This can be a fatal flaw as training a model with repeated elements can outweight other annotation or completely confuse it. Let's see if we have duplicates elements. 
...
:::{.callout-note title="Model description" collapse="false"}
...
:::{.callout-note title="Tips and tricks - Read the model configuration" collapse="false"}
...
Tokenizing consists of two steps, first text patterns are transformed into tokens from the model's vocabulary, then each sentence is truncated or padded so that each entry has the same length. 
...
Here we can see that the model performance for the labels "agrees" and "neutral" is decent, but would benefit from further hyperparameter tuning. On the other hand, the model does not recognise the "disagrees" label. All in all, the current finetuning leads to mediocre results. We will tweak the hyperparameters to reach better results. The first step is reading the learning curve.


## Conclusion 

In this section we have covered the basics to load the data and model, launch the fine-tuning it and evaluate the performances. We discussed: 
...
for batch in dsd["test"].batch(batch_size=16, drop_last_batch=False):
  print(batch["labels"]); break
  model_input = {'input_ids' : batch['input_ids'],'attention_mask' : batch['attention_mask']}
  logits : np.ndarray = model(**model_input).logits.detach().cpu().numpy()
  output += [
    {
      "ID": str(id_), 
      "GS": int(np.argmax(labels_).item()), 
      "PRED": int(np.argmax(logit_row).item())
    }
    for id_, labels, logit_row in zip(batch["ID"].cpu(), batch["labels"].cpu(), logits)
  ]
(
  pd.DataFrame({
    "ID": text_id,
...
TypeError: Got unsupported ScalarType BFloat16 -> when using bfloat16 and .numpy() -> .float().numpy()
