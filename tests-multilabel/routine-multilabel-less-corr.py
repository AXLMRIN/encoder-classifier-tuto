...
for n_classes in range(2, len(CLASSES) + 1): 
    output_dir = f"./models/multilabel-bottom-{n_classes}"
    ...
    pipe.train(dsd)
    config = pipe.save_important_info(f"./model-configs/multilabel-bottom-{n_classes}.json")
    ...
    for checkpoint in os.listdir(output_dir):
        ...
        for split in predictions:
            predictions[split].to_csv(f"./outputs/multilabel-bottom-{n_classes}-{split}.csv")
